...
document.querySelectorAll('.mode-tab').forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    document.getElementById(`${currentMode === 'clock' ? 'clock' : currentMode === 'stopwatch' ? 'sw' : 'timer'}View`).classList.add('active');
...
function renderControls() {
  ...
  if (currentMode === 'clock') {
    controlsArea.innerHTML = `<span style="font-size: 0.75rem; color: var(--text-muted); align-self: center;"><i class="fa-solid fa-bolt"></i> Live Local Time</span>`;
